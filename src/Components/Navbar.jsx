      ...
      <div className='max-w-6xl mx-auto flex justify-between items-center'>
        ...
        <div className='flex gap-7 items-center'>
          ...
          </div>
          {
            ...
            border-2 p-5 border-grey-100 rounded-md'>
              <h1 className='font-semibold mb-4 text-xl flex justify-between'>Change Location <span onClick={toggleDropdown}><CgClose/></span></h1>
              <button onClick={getLocation} className='bg-red-500 text-white px-3 pt-1 rounded-md cursor-pointer hover:bg-red-400'>Detect my location</button>
            </div> : null
          }
        </div>
        {/*menu section*/}
        <nav className='flex gap-7 items-center'>
          ...
          <Link to={"/cart"} className='relative'>
            ...
            <span className='bg-red-500 px-2 rounded-full absolute -top-3 -right-3 text white text-sm'>{cartItem.length}</span>
          </Link>
